...
namespace LitJson
{
    ...
    public class JsonMapper
    {
            ...
            else if (reader.Token == JsonToken.ObjectStart)
            {
                ...
                while (true)
                {
                    ...
                    string property = (string) reader.Value;
                     ...
                     //TODO:此处通过反射属性获取字段来解析，以解决变量命名和配置表或通信json中命名规范不同的问题。但若解析大量配置时遇到性能问题，则应考虑调整做法
                     var find = false;
                     foreach (var propPair in t_data.Properties)
                     {
                         var prop_data = propPair.Value;
                         var jsonMemberAttribute = prop_data.Info.GetCustomAttribute<JsonMemberAttribute>();
                         if (jsonMemberAttribute != null && jsonMemberAttribute.Name == property)
                         {
        ...
        public static unsafe StackObject* JsonToObject(ILIntepreter intp, StackObject* esp, IList<object> mStack,
