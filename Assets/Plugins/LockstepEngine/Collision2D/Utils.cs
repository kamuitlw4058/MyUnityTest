...
//https://stackoverflow.com/questions/1073336/circle-line-segment-collision-detection-algorithm
        ...
        public static bool TestRayOBB(LVector2 origin, LVector2 direction, LVector2 center, LVector2 size, LFloat deg, out LFloat tmin){
            var fo = origin - center;
            fo = fo.Rotate(deg);
            var fd = direction.Rotate(deg);
            return TestRayAABB(fo, fd, -size, size, out tmin);
        }

        public static bool TestRayAABB(LVector2 origin, LVector2 direction, LVector2 min, LVector2 max, out LFloat tmin){
            ...
            // For all three slabs
            for (int i = 0; i < 2; i++) {
                if (Abs(direction[i]) < LFloat.EPSILON) {
                    // Ray is parallel to slab. No hit if origin not within slab
                    if (origin[i] < min[i] || origin[i] > max[i]) return false;
                }
                else {
                    // Compute intersection t value of ray with near and far plane of slab
                    LFloat ood = LFloat.one / direction[i];
                    LFloat t1 = (min[i] - origin[i]) * ood;
                    LFloat t2 = (max[i] - origin[i]) * ood;
        ...
        public static bool TestSegmentOBB(LVector2 p1, LVector2 p2, LVector2 center, LVector2 size, LFloat deg){
            var fp1 = p1 - center;
            fp1 = fp1.Rotate(deg);
            var fp2 = p2 - center;
            fp2 = fp2.Rotate(deg);
            return TestSegmentAABB(fp1, fp2, -size, size);
        }
        
        public static bool TestSegmentAABB(LVector2 p1, LVector2 p2, LVector2 min, LVector2 max){
            LFloat tmin = LFloat.zero;
            LFloat tmax = LFloat.one;

            LVector2 direction = new LVector2(p2[0] - p1[0], p2[1] - p1[1]);
            for (var i = 0; i < 2; i++)
            {
                if (Abs(direction[i]) < LFloat.EPSILON)
                {
                ...
                {
                    // Compute intersection t value of ray with near and far plane of slab
                    //TODO:这里乘除1000用来弥补定点数精度不足问题
                    LFloat ood = LFloat.one * 1000 / direction[i];
                    LFloat t1 = (min[i] - p1[i]) * ood * new LFloat(true, 1);
                    LFloat t2 = (max[i] - p1[i]) * ood * new LFloat(true, 1);
                    if (t1 > t2)
                    {
                        LFloat tmp = t1; 
                        t1 = t2; 
